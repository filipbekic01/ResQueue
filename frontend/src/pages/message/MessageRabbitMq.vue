...
<template>
  <div class="mx-5 my-3 rounded-lg">
    ...
    <div
      ...
      :key="key"
      class="even:bg-slate-100/50 rounded"
    >
      <div v-if="key === 'properties'" class="bg-white rounded-lg">
        <div class="font-semibold bg-slate-100 my-1 px-2 rounded-lg">properties</div>
        <div
          v-for="(pValue, pKey) in rabbitMqMessage?.parsed.properties"
          :key="pKey"
          class="even:bg-slate-100/50 rounded ms-4"
        >
          <div v-if="pKey === 'headers'" class="bg-white rounded-lg">
            <div class="font-semibold bg-slate-100 my-1 px-2 rounded-lg">headers</div>
            <div
          ...
          <div v-else class="flex">
            ...
            <div class="text-slate-500">{{ pValue }}</div>
          </div>
        </div>
      </div>
      <div v-else class="flex">
